...
/* 
* added nullptr assignment and comparison. 
* Try Catch block deactivated (in Arduino
* unnecessary)
**/
        ...
        if (NULL != p)
        {
            if (NULL == pn)
            {
                ...
                pn = new long(1); // may throw std::bad_alloc
                                  //in a avr uC, if here is an error error handling is senseless
                //}
                //catch (std::bad_alloc&)
                //{
                //    delete p;
...
template <typename T>
bool operator!=(const shared_ptr<T> &l, nullptr_t)
{
    return (bool)l;
}
...
/// shared_ptr comparison with nullptr
template <typename T>
bool operator!=(nullptr_t, const shared_ptr<T> &l)
{
    return (bool)l;
}
